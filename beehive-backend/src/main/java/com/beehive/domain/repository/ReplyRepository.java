package com.beehive.domain.repository;
...
public interface ReplyRepository extends JpaRepository<ReplyEntity, Long> {
    ...
    @Query(
            "SELECT r.replyId AS replyId, r.replyBody AS replyBody, r.repliedBy AS repliedBy, r.repliedOn AS repliedOn, r.numberOfLikes AS numberOfLikes FROM ReplyEntity r WHERE r.replyId = :replyId")
    Optional<ReplyEntityProjection> findReplyByReplyId(@Param("replyId") Long replyId);

    @Query(
            "SELECT r.replyId AS replyId, r.replyBody AS replyBody, r.repliedBy AS repliedBy, r.repliedOn AS repliedOn, r.numberOfLikes AS numberOfLikes FROM ReplyEntity r WHERE r.post.postId = :postId")
